	...
	function(file, record, search,runtime,https,email) {
		function execute(options){  
			...
			var headers={
				...
				var content='Name,Department,Operating Area,Supervisor,Job Title,Primary Earning Type\n';
				...
				//{
					...
					for(var s=0;s<splitTotalLinks.length;s++)
					{
						...
						//var content='Department,Operating Area,Supervisor,Job Title,Primary Earning Type\n';
						var eName=removeCommas(empData.data[0].employee_name);
						var department=removeCommas(empData.data[0].department_description);
						var opArea=removeCommas(empData.data[0].position_family);
						var supervisor=removeCommas(empData.data[0].supervisor_primary);
						var jobTitle=removeCommas(empData.data[0].business_title);
						var PET=removeCommas(empData.data[0].cat2desc);
						//var thisSupervisor=thisData.data[0].deptdesc;
						//var thisJob=thisData.data[0].deptdesc;
						//var thisPET=thisData.data[0].cat4desc;
						content=content+eName+','+department+','+opArea+','+supervisor+','+jobTitle+','+PET+'\n';
						var dkdkfld=99;
						}
						}
						catch(err)
