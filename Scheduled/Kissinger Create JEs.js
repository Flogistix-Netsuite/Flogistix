...
define(["N/https", "N/record", "N/log", "N/runtime", "N/search"], (
  ...
  const getWorkOrders = (createdJEs, existingWorkOrders, workOrderIssues) => {
    // const params = "$filter=not%20customFields/Any(c:(c/value%20eq%20'NO%20PARTS'%20or%20startswith(c/value,'JE'))%20and%20c/fieldname%20eq%20'API%20Loaded')%20and%20statusId%20eq%20'Completed'";
    // const params = "$filter=not%20customFields/Any(c:(c/value%20eq%20'IMPORTED'%20or%20c/value%20eq%20'NO%20PARTS')%20and%20c/fieldname%20eq%20'API%20Loaded')%20and%20statusId%20eq%20'Completed'";
    //   "$filter=not%20customFields/Any(c:c/value%20eq%20'IMPORTED'%20and%20c/fieldname%20eq%20'API%20Loaded')%20and%20statusId%20eq%20'Completed'";
    // const params = "$filter=customFields/Any(c:(c/value%20eq%20''%20or%20c/value%20eq%20null)%20and%20c/fieldname%20eq%20'API%20Loaded')%20and%20statusId%20eq%20'Completed'";
    const params =
      "$filter=(customFields/Any(c:(c/value%20eq%20''%20or%20c/value%20eq%20null)%20and%20c/fieldname%20eq%20'API%20Loaded')%20or%20not%20customFields/Any(c:c/fieldname%20eq%20'API%20Loaded'))%20and%20statusId%20eq%20'Completed'";

    const workOrders = managerPlusGet("WorkOrders/", params);
    if (workOrders) {
      log.debug("Work Orders Retrieved", workOrders);
      workOrders.forEach((wo) => {
        ...
        parts.forEach((part) => {
          log.debug({title:'hello',details:part});
          let truncatedUnitCost = Math.trunc(part.unitCost * 100) / 100;
        ...
        if (!woUpdateRes) {
          ...
          workOrderIssues.push({
            workOrderNumber: wo.workOrderNumber,
            error: `could not mark WO ${wo.workOrderKey} as API Loaded. This order will be duplicated next run if you do not mark it by hand in Manager+`,
          });
        }
  ...
  const execute = (context) => {
    try {
      ...
      var workOrderIssues = [];
      getWorkOrders(createdJEs, existingWorkOrders, workOrderIssues);
    } catch (e) {
      log.error("Script Execution Error", `Error: ${e.name} - ${e.message}`);
      throw e;
    } finally {
      log.audit({
        title: "JE Creation Summary",
        details: "Created JEs: " + JSON.stringify(createdJEs),
      });
      log.audit({
        title: "Existing Work Orders already in system",
        details:
          "Existing Work Orders already in system: " +
          JSON.stringify(existingWorkOrders),
      });
      log.audit({
        title: "JEs Not Created Summary",
        details: "Work Order issues: " + JSON.stringify(workOrderIssues),
      });
    }
  };
  return {
